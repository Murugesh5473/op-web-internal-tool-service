...
export const useTestCaseDetail = ({ testRunId, testCaseId, initialTc, initialSpec, initialSuite, initialAllTcs } = {}) => {
  ...
  useEffect(() => {
    if (initialTc && initialSpec && initialSuite) {
      setData({ tc: initialTc, spec: initialSpec, suite: initialSuite, allTcs: initialAllTcs });
    }
  }, [initialTc, initialSpec, initialSuite, initialAllTcs]);
